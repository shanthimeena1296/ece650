...
std::vector<int> find_vertexcover(int, vector<int>, int k);
...
Graph readGraph(vector<int> vec_V, vector<int>vec_E)
{
	...
	Graph graph;
	int i;
    //no idea why this loop is sitting here

	for (i = 0; i < MAX; i++)
	{
		graph.node[i].v = i; //assigning v value 
...
void vertexcover(int vertex_num, vector<int>edge) {
    // std::cout<<g<<std::endl;
	Solver solver;
    ...
    for (auto x:tmp)
    {
        std::cout<<"-------tmp value alocation--------"<<x<<std::endl;
    }

	while (hi >= low) {
		int mid = (hi + low) / 2;
        // std::cout<<num<<"----debug182----"<<std::endl;
        // std::cout<<vec_edge<<"----debug183----"<<std::endl;
...
std::vector<int> find_vertexcover(int vertex_num, vector<int>edge, int k) {
	...
	if (solver.solve()) {
        ...
        for (int i: vertex_cover){
                std::cout << i << ' -- vertex cover'<<std::endl;
        }
      
        // std::cout<<vertex_cover<<std::endl;
		return vertex_cover;
		//sort	
	}
	else {

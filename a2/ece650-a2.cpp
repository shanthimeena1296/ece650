...
class create_graph {
  ...
  explicit create_graph(int vert) {
    this -> vert = vert;
    std::cout<<vert<<" ----debug19 ---- vert"<<std::endl;
    this -> adj_l = new std::vector < int > [vert + 1];
    std::cout<<adj_l<<" ----debug21 ---- adj_l"<<std::endl;
  ...
  while (!std::cin.eof()) {
    ...
    try{
      if (line.find('V') == 0) {
        ...
        std::cout<<undirected_graph<<" ----debug110 ---- undirected graph"<<std::endl;
        undirected_graph = new create_graph(v);  
        std::cout<<undirected_graph<<" ----debug112 ---- undirected graph"<<std::endl;
      }
      if (line.find('E') == 0) {
        std::size_t spec_char = line.find('<');
        std::cout<<spec_char<<" ----debug116 ----  spec_char"<<std::endl;
        int flag = 0;
        ...
        if (flag == 0) {
          ...
          std::cout<<spec_char2<<" ----debug132 ----  spec_char2"<<std::endl;
